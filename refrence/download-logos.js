...
async function downloadAllLogos() {
  console.log(`\n🚀 Starting to download ${data.totalLogos} SVG logos for ${data.totalBrands} brands...\n`);
  ...
  for (const brand of data.brands) {
    if (brand.logos.length === 0) {
      console.log(`⊘ ${brand.brandName}: No logos to download`);
      continue;
    }
    
    // Create directory for this brand (using domain name)
  ...
  console.log('═══════════════════════════════════════════════════════════\n');
  console.log(`🎉 Done! Check the "${mainDir}" folder for all logos organized by domain.\n`);
}
